...
{
    // switches
    private readonly List<Description> _switches = new ();
    // args
    private readonly List<Description> _args = new ();
    private readonly List<Description> _optionalArgs = new ();
    ...
    public Define<T> OptionalArgument<TA>(Expression<Func<T, TA>> expression, string longName, char? shortName = null)
    {
    ...
    private static void AddDescription<TA>(List<Description> memberCollection, IReadOnlyList<Description> validateOnCollection, 
        Expression<Func<T, TA>> expression, string longName, char? shortName)
    {
        ...
        Validate.Definition(description, validateOnCollection, typeof(TA) == typeof(bool) ? new []{ typeof(bool)} : null);
        memberCollection.Add(description);
    }

    public ParsedArgs<T?> ParseArgs(string[] cliArgs, bool allPropertiesMustBeDefined = true, bool ignoreAllPropertiesSet = false)
    {
